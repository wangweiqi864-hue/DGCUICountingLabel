...
![alt text](https://github.com/dataxpress/DGCUICountingLabel/blob/master/demo.gif "demo")

## CocoaPods ######
DGCUICountingLabel is available on CocoaPods.
Add this to your Podfile:

`pod 'DGCUICountingLabel'`
...
## Setup ######
Simply initialize a `DGCUICountingLabel` the same way you set up a regular `UILabel`:

    DGCUICountingLabel* myLabel = [[DGCUICountingLabel alloc] initWithFrame:CGRectMake(10, 10, 100, 40)];
    [self.view addSubview:myLabel];

You can also add it to your XIB file, just make sure you set the class type to `DGCUICountingLabel` instead of `UILabel` and be sure to `#import "DGCUICountingLabel.h"` in the header file.
...
Alternatively, you can provide a `DGCUICountingLabelFormatBlock`, which permits greater control over how the text is formatted:
...
There is also a `DGCUICountingLabelAttributedFormatBlock` to use an attributed string. If the `formatBlock` is specified, it takes precedence over the `format`.
...
## CocoaPods ######
DGCUICountingLabel 可以使用cocoaPods导入,
添加以下代码到你的Podfile文件:
...
## 设置 ######
初始化 `DGCUICountingLabel` 的方式和普通的 `UILabel`是一样的:

    DGCUICountingLabel* myLabel = [[DGCUICountingLabel alloc] initWithFrame:CGRectMake(10, 10, 100, 40)];
    [self.view addSubview:myLabel];

你也可以用在 XIB 文件中, 前提是你在头文件中引入了 `DGCUICountingLabel`的头文件并且使用 `DGCUICountingLabel`替换掉了原生的`UILabel`.
...
另外，你也可以使用 `DGCUICountingLabelFormatBlock`, 这个可以对显示的文本格式进行更加高度的自定义:
...
除此之外还有一个 `DGCUICountingLabelAttributedFormatBlock` 用于设置属性字符串的格式,用法和上面的block类似. 如果指定了以上两个 `formatBlock`中的任意一个 , 它将会覆盖掉 `format`属性,因为block的优先级更高.
